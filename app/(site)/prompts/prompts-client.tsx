      ...
      <section className="mt-8 grid gap-6 lg:grid-cols-[minmax(0,1fr)_22rem]">
        <div className="space-y-5">
          <div className="grid gap-3 border-b border-border-subtle pb-4 sm:grid-cols-2 xl:grid-cols-5">
            <select aria-label="Filter by category" value={category} onChange={(event) => { setCategory(event.target.value); setPage(1); }} className={fieldClass()}>
              {categoryOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}
            </select>
            <select aria-label="Filter by AI model" value={model} onChange={(event) => { setModel(event.target.value); setPage(1); }} className={fieldClass()}>
            ...
            <select aria-label="Filter by difficulty" value={difficulty} onChange={(event) => { setDifficulty(event.target.value); setPage(1); }} className={fieldClass()}>
              {difficultyOptions.map((option) => <option key={option} value={option}>{option === "all" ? "Difficulty" : option}</option>)}
            </select>
            <select aria-label="Filter by prompt type" value={promptType} onChange={(event) => { setPromptType(event.target.value); setPage(1); }} className={fieldClass()}>
