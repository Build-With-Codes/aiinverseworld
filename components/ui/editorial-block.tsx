...
export function EditorialBlock({ eyebrow, title, tone = "info", children }: EditorialBlockProps) {
  return (
    ...
    >
      <div className="absolute inset-y-0 left-0 w-1 bg-gradient-to-b from-brand-cyan to-brand-violet" aria-hidden />
      {eyebrow ? <p className="text-eyebrow text-brand-cyan-strong">{eyebrow}</p> : null}
